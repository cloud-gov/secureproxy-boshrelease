...
function _M.check_ingress(opts)
  ...
  -- Note: This is only safe to use behind a load balancer or trusted device that appends the client IP to XFF;
  ...
  if xff ~= nil then
    ...
    if xffl ~= nil then
      ...
      -- 2. check last value in X-Forwarded-For against global_whitelist
  ...
  -- if passed tic_secret check, trusted_ip must match global_whitelist
  if trusted_ip ~= nil and not ip_in_cidrs(trusted_ip, opts["global_whitelist"]) then
    return false, true, trusted_ip, email
  end
